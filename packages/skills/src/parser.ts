...
export async function parseRuleFile(
  ...
  // Parse impact
  const impactMatch = bodyContent.match(/\*\*Impact:\s*(CRITICAL|HIGH|MEDIUM|LOW)/i)
  const impact = (impactMatch ? impactMatch[1].toUpperCase() : 'MEDIUM') as ImpactLevel

  // Parse impact description
  const impactDescMatch = bodyContent.match(/\*\*Impact:\s*\w+\s*\(([^)]+)\)/)
  const impactDescription = impactDescMatch ? impactDescMatch[1].trim() : undefined

  // Parse explanation (text before first code block or "**Incorrect")
  const explanationMatch = bodyContent.match(/##\s+.+?\n\n([\s\S]*?)(?=\n\n\*\*Incorrect|```)/m)
  ...
  const exampleRegex = /\*\*(Incorrect|Correct)(?:\s*\([^)]+\))?:\*\*\s*\n\n```(\w+)\n([\s\S]*?)```/g
  let match
