...
import { analyzeHallucinationRisk } from './analyzer';
import { calculateHallucinationScore } from './scoring';
import type { HallucinationRiskOptions } from './types';
...
program
  .name('aiready-hallucination-risk')
...
EXAMPLES:
  aiready-hallucination-risk .                        # Full scan
  aiready-hallucination-risk src/ --output json       # JSON report
  aiready-hallucination-risk . --min-severity major   # Only major+
`)
  ...
  .action(async (directory, options) => {
    console.log(chalk.blue('🧠 Analyzing hallucination risk...\n'));
    ...
    const finalOptions: HallucinationRiskOptions = {
      rootDir: directory,
    ...
    const report = await analyzeHallucinationRisk(finalOptions);
    const scoringOutput = calculateHallucinationScore(report);
    ...
    if (options.output === 'json') {
      ...
      const outputPath = resolveOutputPath(
        options.outputFile,
        `hallucination-risk-report-${new Date().toISOString().split('T')[0]}.json`,
        directory,
      );
...
function displayConsoleReport(report: any, scoring: any, elapsed: string) {
  ...
  if (summary.totalSignals === 0) {
    console.log(chalk.green('✨ No hallucination risk signals found! Your codebase is AI-safe.\n'));
    return;
  }
