...
function usage() {
  console.log(
    `Usage: node scripts/visualize.js [directory] [--report <report.json>] [--output <out.html>] [--open]`
  );
  process.exit(1);
}
...
function findLatestReport(dir) {
  ...
  const files = readdirSync(aireadyDir).filter(
...
try {
  if (!reportPath || !existsSync(reportPath)) {
    ...
    try {
      execSync(
        process.platform === 'win32' ? 'where aiready' : 'command -v aiready',
        { stdio: 'ignore' }
      );
      cliCmd = 'aiready';
    ...
    const autoReportPath = resolve(
      dir,
      '.aiready/aiready-report-' +
        new Date()
          .toISOString()
          .replace(/[^0-9]/g, '')
          .slice(0, -3) +
